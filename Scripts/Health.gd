...
func _on_Health_body_entered(body: Node) -> void:
  ...
  print(get_tree().root.get_node(cur_lvl).get_node("Player").hp)
  get_tree().root.get_node(cur_lvl).get_node("CanvasLayer2").get_node("Player_UI").get_node("Healthbar").value = get_tree().root.get_node("Graveyard_Level").get_node("Player").hp
  queue_free()
